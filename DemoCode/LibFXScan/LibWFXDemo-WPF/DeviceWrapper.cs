...
class DeviceWrapper
{
    ...
    public String REGISTER_EXENAME = @"RegisterAP.exe";
    public String m_szSdkInstallPath = @"";
    ...
    [UnmanagedFunctionPointer(CallingConvention.StdCall, CharSet = CharSet.Unicode)]
    public delegate ENUM_LIBWFX_ERRCODE LibWFX_GetLastErrorCode(ENUM_LIBWFX_ERRCODE enErrorCode, IntPtr szErrorMsg);
    ...
    public DeviceWrapper()
    {
        ...
        if (hLibModule == IntPtr.Zero || hCommandModule == IntPtr.Zero)
        {
            ...
            if (Environment.Is64BitOperatingSystem && !Environment.Is64BitProcess)
                m_szSdkInstallPath = (string)Registry.GetValue("HKEY_LOCAL_MACHINE\\SOFTWARE\\WOW6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{E96A9957-0A5A-40C3-8358-75A3FA6D9CC7}_is1", "InstallLocation", "");
            else
                m_szSdkInstallPath = (string)Registry.GetValue("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{E96A9957-0A5A-40C3-8358-75A3FA6D9CC7}_is1", "InstallLocation", "");          

            if (Directory.Exists(szPath) && Directory.Exists(m_szSdkInstallPath))
            {
                ...
                if (m_szSdkInstallPath.LastIndexOf('\\') != (m_szSdkInstallPath.Length - 1))
                    m_szSdkInstallPath += "\\";
