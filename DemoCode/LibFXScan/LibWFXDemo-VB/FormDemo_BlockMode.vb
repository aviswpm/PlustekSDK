...
#End If
            ...
            ElseIf enRet = ENUM_LIBWFX_ERRCODE.LIBWFX_ERRCODE_NO_DOC_OCR Then
                ...
                GetCertificatePermission()
            Else
                If enRet = ENUM_LIBWFX_ERRCODE.LIBWFX_ERRCODE_PATH_TOO_LONG Then
    ...
    Private Sub BTN_REFRESH_Click(sender As Object, e As EventArgs) Handles BTN_REFRESH.Click
        ...
        If enRet = ENUM_LIBWFX_ERRCODE.LIBWFX_ERRCODE_SUCCESS Then
            ...
            For idx As Integer = 0 To listSerialNumber.Count - 1
                WriteLog("Device: " + listDevice(idx) + "   " + "Serial Number: " + listSerialNumber(idx))
            Next
        ElseIf enRet = ENUM_LIBWFX_ERRCODE.LIBWFX_ERRCODE_NO_INIT Or enRet = ENUM_LIBWFX_ERRCODE.LIBWFX_ERRCODE_LOAD_MRTD_DLL_FAIL Or enRet = ENUM_LIBWFX_ERRCODE.LIBWFX_ERRCODE_SCANNING Then
            WriteLog("[ Warning ] " + enRet.ToString() + " - [" + Convert.ToDecimal(enRet).ToString() + "]")
        Else
            WriteLog("[ Warning ] " + ENUM_LIBWFX_ERRCODE.LIBWFX_ERRCODE_NO_DEVICES.ToString() + " - [" + Convert.ToDecimal(ENUM_LIBWFX_ERRCODE.LIBWFX_ERRCODE_NO_DEVICES).ToString() + "]")
        End If
    End Sub
    ...
    Private Sub PrintImg(ByVal szPath As String)
        If Not IO.File.Exists(szPath) Then Exit Sub
    ...
    Private Sub BTN_CALIBRATE_Click(sender As Object, e As EventArgs) Handles BTN_CALIBRATE.Click
        If CMB_DEVICE_LIST.Items.Count = 0 Then
            WriteLog("[ Warning ] " + ENUM_LIBWFX_ERRCODE.LIBWFX_ERRCODE_NO_DEVICES.ToString() + " - [" + Convert.ToDecimal(ENUM_LIBWFX_ERRCODE.LIBWFX_ERRCODE_NO_DEVICES).ToString() + "]")
            Return
        End If
    ...
    Private Sub BTN_SCAN_Click(sender As Object, e As EventArgs) Handles BTN_SCAN.Click
        ...
        If enRet <> ENUM_LIBWFX_ERRCODE.LIBWFX_ERRCODE_SUCCESS Then
            ...
            DispatcherWriteLog("[ Warning ] " + pszErrorMsg2 + " - [" + Convert.ToDecimal(enRet).ToString() + "]") REM get fail message
        ElseIf szEventRet.Length > 1 Then
            ...
            If Command.IndexOf("""rawdata"":true") <> -1 Then
                ...
                End If
            Else
                Dim nMaxLength As Integer = Math.Max(ScanImageArray.Length, OCRResultArray.Length)
                If nMaxLength > 0 Then
                    For idx As Integer = 0 To nMaxLength - 1
                        If idx < ScanImageArray.Length - 1 Then
                            DispatcherWriteLog(ScanImageArray(idx).Trim())  REM get each image path
                            If ((ScanImageArray(idx).Contains(".jpg") = True Or ScanImageArray(idx).Contains(".png") = True Or ScanImageArray(idx).Contains(".bmp") = True) And ScanImageArray(idx).ToUpper.Contains("_PHOTO") <> True And String.IsNullOrEmpty(ScanImageArray(idx).Trim()) = False) Then
                                DispatcherPrintImg(ScanImageArray(idx).Trim())
                            End If
                        End If
                        If idx < OCRResultArray.Length - 1 Then
    ...
    Private Sub BTN_REGISTER_Click(sender As Object, e As EventArgs) Handles BTN_REGISTER.Click
        ...
        startInfo.FileName = m_DeviceWrapper.m_szSdkInstallPath + REGISTER_EXENAME
        Try
